...
echo -e "${BLUE}🚀 Быстрое развертывание Детектора подозрительных менеджеров${NC}"
...
# Создание index.html
cat > /var/www/detector/index.html << 'EOF'
<!DOCTYPE html>
<html lang="ru">
<head>
    <meta charset="UTF-8">
...
cat > /var/www/detector/styles.css << 'EOF'
/* Базовые стили */
* {
    margin: 0;
    padding: 0;
...
cat > /var/www/detector/script.js << 'EOF'
...
mkdir -p /var/www/detector/Gif
mkdir -p /var/www/detector/Music

# Создание заглушек для ресурсов
echo "<!-- Заглушка для GIF -->" > /var/www/detector/Gif/placeholder.txt
echo "<!-- Заглушка для MP3 -->" > /var/www/detector/Music/placeholder.txt
...
cat > /var/www/detector/server.js << 'EOF'
...
echo -e "  Статус сервиса: ${BLUE}sudo systemctl status detector${NC}"
echo -e "  Логи сервиса: ${BLUE}sudo journalctl -u detector -f${NC}"
echo -e "  Перезапуск: ${BLUE}sudo systemctl restart detector${NC}"
...
echo -e "  ${BLUE}sudo certbot --nginx -d home.com${NC}"
