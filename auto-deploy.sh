...
echo -e "${BLUE}🚀 Автоматическое развертывание Детектора подозрительных менеджеров${NC}"
echo -e "${BLUE}Сервер: 217.196.100.86${NC}"
echo -e "${BLUE}Домен: home.com${NC}"
echo ""
...
echo -e "${YELLOW}👤 Создание пользователя detector...${NC}"
if ! id "detector" &>/dev/null; then
    useradd --system --group --home /var/www/detector detector
    echo -e "${GREEN}✅ Пользователь detector создан${NC}"
else
    echo -e "${YELLOW}⚠️  Пользователь detector уже существует${NC}"
fi
...
# Создание styles.css (сокращенная версия)
cat > styles.css << 'EOF'
/* Базовые стили */
* {
    margin: 0;
...
# Создание script.js (сокращенная версия)
cat > script.js << 'EOF'
...
const mockChannels = {
    'suspicious': {
        ...
        score: 85,
        factors: ['Слишком много рекламы', 'Подозрительные ссылки', 'Низкое качество контента'],
        recommendations: ['Будьте осторожны с этим каналом', 'Не переводите деньги без проверки']
    },
    'legitimate': {
        name: 'Легитимный канал',
        score: 15,
        factors: ['Качественный контент', 'Прозрачная информация', 'Положительные отзывы'],
        recommendations: ['Канал выглядит надежно', 'Можете доверять информации']
    },
    'mixed': {
        name: 'Смешанный канал',
        score: 55,
        factors: ['Некоторые подозрительные элементы', 'Частично достоверная информация'],
        recommendations: ['Будьте осторожны', 'Проверяйте информацию дополнительно']
    }
};
...
# Установка прав и запуск сервисов
...
if systemctl is-active --quiet detector; then
    echo -e "${GREEN}✅ Сервис detector запущен${NC}"
else
    echo -e "${RED}❌ Ошибка запуска сервиса detector${NC}"
...
# Получение IP адреса
SERVER_IP=$(curl -s ifconfig.me 2>/dev/null || echo "217.196.100.86")

echo -e "${GREEN}🎉 Развертывание завершено!${NC}"
echo -e "${BLUE}📱 Сайт доступен по адресу: http://$SERVER_IP${NC}"
echo -e "${BLUE}📱 Домен: http://home.com${NC}"
echo ""
echo -e "${YELLOW}📋 Полезные команды:${NC}"
echo -e "  Статус сервиса: ${BLUE}systemctl status detector${NC}"
echo -e "  Логи сервиса: ${BLUE}journalctl -u detector -f${NC}"
echo -e "  Перезапуск: ${BLUE}systemctl restart detector${NC}"
echo -e "  Логи Nginx: ${BLUE}tail -f /var/log/nginx/access.log${NC}"
echo ""
echo -e "${YELLOW}🔒 Для настройки SSL сертификата:${NC}"
echo -e "  ${BLUE}apt install certbot python3-certbot-nginx${NC}"
echo -e "  ${BLUE}certbot --nginx -d home.com${NC}"
